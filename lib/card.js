...
function openModal(index) {
  ...
  modal.innerHTML = \`
    ...
    \${card.latest_news ? \`<div style="background:#f0f9ff;padding:10px 14px;border-radius:8px;font-size:0.85em;color:#1e40af;margin:12px 0">📰 <strong>Latest:</strong> \${card.latest_news}</div>\` : ''}

    <h2>Current Focus & Goals</h2>
    \${card.current_focus ? \`<p><strong>🎯 Focus:</strong> \${card.current_focus}</p>\` : ''}
    ...
    <h2>Personality</h2>
    <p><strong>MBTI:</strong> <span title="\${card.mbti_reason || ''}" style="cursor:help;border-bottom:1px dashed #94a3b8">\${card.mbti || '?'}</span>\${card.mbti_reason ? \` — \${card.mbti_reason}\` : ''}</p>
    ...
    \${card.last_connection ? \`<p><strong>🤝 Last connection:</strong> \${card.last_connection}</p>\` : ''}
