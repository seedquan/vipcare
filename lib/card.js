...
function openPanel(index) {
  ...
  const tweets = [];
  if (card.raw_tweets) {
    const tweetBlocks = card.raw_tweets.split(/─{5,}/);
  ...
  if (card.recent_activity) {
    ...
    for (const line of lines) {
      ...
      if (dateMatch) {
        ...
        activityItems[activityItems.length - 1].text += line.replace(/^[-•*]\s*/, '').trim() + ' ';
      } else {
        activityItems.push({ date: '', text: line.replace(/^[-•*]\s*/, '').trim() });
      }
    }
  }
  ...
  panel.innerHTML = \`
    ...
    <div class="tab-content active" id="tab-recent">
      ...
      \${tweets.length ? \`<h2>Latest Tweets</h2>\${tweets.slice(0,5).map(t => \`<div class="tweet">\${t.text}\${t.date ? \`<div class="tweet-date">\${t.date}\${t.link ? \` · <a href="\${t.link}" target="_blank" style="color:#2563eb">link</a>\` : ''}</div>\` : ''}</div>\`).join('')}\` : ''}

      \${activityItems.length ? \`<h2>Recent Moves</h2>\${activityItems.slice(0,8).map(a => \`<div class="activity-item">\${a.date ? \`<span class="activity-date">\${a.date}</span> · \` : ''}\${a.text.trim()}</div>\`).join('')}\` : ''}
